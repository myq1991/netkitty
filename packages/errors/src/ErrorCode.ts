 ...
 * with a stable string `code`. Numbers are grouped by package (1000s = capture, 2000s = codec, …) so a
 * code is globally unique and can be switched on regardless of which package raised it. Concrete error
 * classes reference these entries when setting their `errno`/`code`.
 */
