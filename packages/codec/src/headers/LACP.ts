...
/** One LACPDU TLV: a 1-byte Type, its on-wire 1-byte Length (which counts the 2 header octets too), and its verbatim hex Value. */
type LacpTlv = {type: number, length: number, value: string}
 ...
 */
export class LACP extends BaseHeader {
        ...
        return {
            ...
            properties: {
                ...
                tlvs: {
                    ...
                    decode: function (this: LACP): void {
                    ...
                    encode: function (this: LACP): void {
                        const tlvs: LacpTlv[] = this.instance.tlvs.getValue([])
                        let offset: number = 2
                        if (tlvs) {
                            for (let i: number = 0; i < tlvs.length; i++) {
                                const tlv: LacpTlv = tlvs[i]
    ...
    public readonly id: string = 'lacp'

    public readonly name: string = 'Link Aggregation Control Protocol'

    public readonly nickname: string = 'LACP'
