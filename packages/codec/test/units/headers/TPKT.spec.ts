...
test('TPKT + COTP DT: framing decode + byte-perfect round-trip', async (): Promise<void> => {
    ...
    // cotp.data; the ISO-Session GIVE-TOKENS/DATA-TRANSFER SPDUs (01 00 01 00) decode as an iso-session
    // layer, and the Presentation/MMS PDU that follows decodes as the mms layer — the full substation
    // stack TPKT/COTP/Session/MMS.
    ...
    assert.strictEqual((Layer(decoded, 'mms').data as any).message, '61093007020103a0020500', 'the Presentation/MMS PDU is the mms layer')
})
